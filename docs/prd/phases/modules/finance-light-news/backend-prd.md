...
定义 `finance-light-news` 模块的后端 API、授权、验证、生命周期、审计和跨端合同，使 Web 与 App 能消费同一服务端事实。
...
- 数据库 PRD：`docs/prd/phases/modules/finance-light-news/database-prd.md`。
...
- 已读取 API 合同基础 PRD。
- 已读取 `finance-light-news` 数据库 PRD。
- 认证会话基础能提供当前用户和角色。
...
1. 员工接口返回财经资讯列表、详情、分类/标签和免责声明。
2. 管理接口支持财经内容草稿、合规检查、发布、归档和下线。
3. 员工接口必须按当前用户权限过滤数据。
...
- API 行为能支持 阅读已发布财经资讯、查看分类/标签和明确的非投资建议提示。
- API 行为能支持 发布、编辑、归档、下线和合规检查财经内容。
