...
定义 `finance-light-news` 模块在 PostgreSQL 16 中需要持久化的业务事实、生命周期、约束、索引方向和样例数据对齐点，为后端 API 和端侧消费提供一致基础。
...
1. 保存文章标题、摘要、正文、来源说明、分类、标签、免责声明、合规标记和内容状态。
2. 内容模型必须支持显式非投资建议提示，并保留合规审核状态。
3. 不得设计股票推荐、买卖建议、个股预测或收益承诺字段。
...
- 数据库需求能支持 阅读已发布财经资讯、查看分类/标签和明确的非投资建议提示。
- 数据库需求能支持 发布、编辑、归档、下线和合规检查财经内容。
