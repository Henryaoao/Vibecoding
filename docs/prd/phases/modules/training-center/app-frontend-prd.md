...
定义 `training-center` 模块在移动端 App 中的独立员工门户行为，使 App 能基于后端合同交付 培训中心 的移动端体验。
...
1. App 提供移动端课程列表、详情、材料入口和仅限本人进度的更新。
