...
定义 `training-center` 模块的后端 API、授权、验证、生命周期、审计和跨端合同，使 Web 与 App 能消费同一服务端事实。
...
- 数据库 PRD：`docs/prd/phases/modules/training-center/database-prd.md`。
...
1. 员工接口返回课程列表、课程详情、材料入口和本人学习进度更新动作。
2. 管理接口支持课程、材料、排序、发布、归档和下线。
3. 员工接口必须按当前用户权限过滤数据。
...
- API 行为能支持 浏览课程、查看材料、更新自己的学习进度。
- API 行为能支持 创建、编辑、发布、归档、下线课程，并维护课程材料和排序。
