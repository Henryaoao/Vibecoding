...
定义 `announcements` 模块的后端 API、授权、验证、生命周期、审计和跨端合同，使 Web 与 App 能消费同一服务端事实。
...
- 数据库 PRD：`docs/prd/phases/modules/announcements/database-prd.md`。
...
1. 员工接口返回可见公告列表、详情、分类筛选和搜索结果。
2. 管理接口支持公告草稿、发布、置顶、有效期、归档和下线。
3. 员工接口必须按当前用户权限过滤数据。
...
- API 行为能支持 查看、筛选和搜索已发布公告。
- API 行为能支持 创建、编辑、置顶、发布、归档、设置有效期和下线公告。
