...
定义 `announcements` 模块在 PostgreSQL 16 中需要持久化的业务事实、生命周期、约束、索引方向和样例数据对齐点，为后端 API 和端侧消费提供一致基础。
...
1. 保存公告标题、正文、分类、置顶标记、有效期、展示范围、附件引用和内容状态。
2. 支持按发布时间、置顶优先级、分类和状态查询。
3. 公告可被归档但不应物理删除已发布历史，除非未来合规策略另行确认。
...
- 数据库需求能支持 查看、筛选和搜索已发布公告。
- 数据库需求能支持 创建、编辑、置顶、发布、归档、设置有效期和下线公告。
