...
定义 `announcements` 模块在 Web 员工门户和 Web Admin Console 中的浏览器端行为，使 Web 能在不重写后端合同的前提下交付 公司公告墙 能力。
...
- 后端 PRD：`docs/prd/phases/modules/announcements/backend-prd.md`。
...
1. Web 员工端提供公告墙、筛选、搜索、详情和置顶展示；Web Admin Console 提供公告管理。
...
- Web 员工端能力能支持 查看、筛选和搜索已发布公告。
- Web Admin Console 能支持 创建、编辑、置顶、发布、归档、设置有效期和下线公告。
