...
定义 `document-center` 模块的后端 API、授权、验证、生命周期、审计和跨端合同，使 Web 与 App 能消费同一服务端事实。
...
- 数据库 PRD：`docs/prd/phases/modules/document-center/database-prd.md`。
...
1. 员工接口返回可见文档列表、详情、搜索/筛选结果和有权限的下载入口。
2. 管理接口支持文档上传元数据、授权、发布、归档、下线和版本管理。
3. 员工接口必须按当前用户权限过滤数据。
...
- API 行为能支持 搜索、筛选、查看文档元数据，并下载自己有权限访问的文档。
- API 行为能支持 上传、编辑、授权、发布、归档和下线文档。
