...
定义 `document-center` 模块在 PostgreSQL 16 中需要持久化的业务事实、生命周期、约束、索引方向和样例数据对齐点，为后端 API 和端侧消费提供一致基础。
...
1. 保存文档元数据、文件引用、分类、标签、版本、权限范围、发布状态和归档状态。
2. 可记录必要的授权下载事件用于安全审计，但不得实现页面浏览历史、最近浏览或被动阅读轨迹。
3. 文件存储位置和对象存储方案属于实现阶段决策，必须与安全/部署约束一起确认。
...
- 数据库需求能支持 搜索、筛选、查看文档元数据，并下载自己有权限访问的文档。
- 数据库需求能支持 上传、编辑、授权、发布、归档和下线文档。
