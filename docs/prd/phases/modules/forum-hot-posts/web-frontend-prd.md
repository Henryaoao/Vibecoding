# ProjectM 模块 PRD：员工论坛热帖 Web 前端层
...
定义 `forum-hot-posts` 模块在 Web 员工门户和 Web Admin Console 中的浏览器端行为，使 Web 能在不重写后端合同的前提下交付 员工论坛热帖 能力。
...
- 后端 PRD：`docs/prd/phases/modules/forum-hot-posts/backend-prd.md`。
...
1. Web 员工端提供热帖列表、详情、评论/互动入口和状态反馈；Web Admin Console 提供热帖治理。
...
- Web 员工端能力能支持 查看热帖、详情、评论摘要，并在后端允许的范围内参与轻量互动。
- Web Admin Console 能支持 维护帖子可见性、状态、排序、违规处理和热度策略参数。
