# ProjectM 模块 PRD：今日公司简报 Web 前端层
...
定义 `today-briefing` 模块在 Web 员工门户和 Web Admin Console 中的浏览器端行为，使 Web 能在不重写后端合同的前提下交付 今日公司简报 能力。
...
1. Web 员工端提供简报列表、详情、今日重点区和历史入口；Web Admin Console 提供简报生命周期管理。
...
- Web 员工端能力能支持 查看当日简报、重点摘要、历史简报和关联入口。
- Web Admin Console 能支持 创建、编辑、排期、发布、置顶、归档和下线简报。
