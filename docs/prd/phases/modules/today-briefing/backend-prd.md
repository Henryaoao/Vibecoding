...
定义 `today-briefing` 模块的后端 API、授权、验证、生命周期、审计和跨端合同，使 Web 与 App 能消费同一服务端事实。
...
- 数据库 PRD：`docs/prd/phases/modules/today-briefing/database-prd.md`。
...
1. 员工接口返回当前可见简报、历史简报列表、详情和关联入口。
2. 管理接口支持简报草稿、排期发布、置顶排序、归档和下线。
3. 员工接口必须按当前用户权限过滤数据。
...
- API 行为能支持 查看当日简报、重点摘要、历史简报和关联入口。
- API 行为能支持 创建、编辑、排期、发布、置顶、归档和下线简报。
