...
定义 `newcomer-zone` 模块在 PostgreSQL 16 中需要持久化的业务事实、生命周期、约束、索引方向和样例数据对齐点，为后端 API 和端侧消费提供一致基础。
...
1. 保存新人资源、任务模板、任务排序、适用范围、任务说明和个人任务完成状态。
2. 个人任务状态只能归属于当前认证用户，管理写入必须通过后端授权控制。
3. 支持资料与任务模板归档，避免删除已被引用的历史任务状态。
...
- 数据库需求能支持 查看新人资料、完成个人入职任务、跟踪自己的任务状态。
- 数据库需求能支持 维护入职资料、任务模板、排序、有效期和面向新人群体的内容状态。
