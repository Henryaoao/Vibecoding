...
import { getContent, getCurrentUser, getHome, logout } from '../services/portal';
import type { ContentItem, CurrentUser, HomePayload, SectionKey } from '../types/portal';
  ...
  useEffect(() => {
    ...
    if (active === 'home') {
      getHome()
        ...
        .catch((err: Error) => !cancelled && handleDataError(err))
  ...
  return (
    <div className={`shell ${menuCollapsed ? 'menu-collapsed' : ''} ${mobileMenuOpen ? 'menu-open' : ''}`}>
      ...
      <aside className="side">
        ...
        {currentUser.roleCode === 'super_user' && (
        ...
        <section className="hero-panel">
          ...
          <div className="hero-actions">
            <div className="current-user" title={currentUser.email}>
              ...
              <small>{currentUser.roleCode}</small>
            </div>
            ...
            <button type="button" className="secondary-action" onClick={handleLogout}>退出</button>
...
function LoginPage() {
  const startTeamsLogin = () => {
    window.location.assign('/api/v1/auth/teams/start');
  };

  return (
    <main className="login-shell">
      <section className="login-card">
        <span className="eyebrow">PROJECTM / SIGN IN</span>
        <h1>CBCX ProjectM</h1>
        <p>使用 Microsoft Teams SSO 登录公司门户。</p>

        <button className="teams-login" type="button" onClick={startTeamsLogin}>
          使用 Teams 登录
        </button>
      </section>
    </main>
